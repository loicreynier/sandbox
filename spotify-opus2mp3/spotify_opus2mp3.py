...
This script uses the FFMPEG library (`ffmpeg-python`) to convert OPUS
audio files provided as command-line arguments into MP3 format.
Spotify does not accept lossless audio formats for local files,
necessitating the convsertion to MP3.

The script uses the mutagen library to improve the formatting
of artist metadata.
In the absence of this metada modification,
multiple artist would appear in the Spotify interface as

    Artist1;Artist2;Artist3;

The rectifies this issue, by concataning the `artist` tag to
a single string, resulting in

    Artist1, Artist2, Artist3

in Spotify interface.

The script will produce MP3 files in the same directory
as the input OPUS files,
maintaining the same file names but with the `.mp3`
extension.
